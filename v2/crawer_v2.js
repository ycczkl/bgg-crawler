...
class bggCrawler {
  ...
  _getGameThumbnail($) {
    let res = [];
    $('.collection_thumbnail a').each(function(i, elem) {
      let childrenType = elem.children[0].name;
      let thumbnail = childrenType === "img" ? elem.children[0].attribs.src : null;
      res.push({thumbnail})
    });
    return res;
  }
  ...
  _getGameMeta($) {
    ...
    $('.collection_thumbnail a').each(function(i, elem) {
      ...
      res.push({bgglink: href, gameId: +href.split('/')[2]})
    });
    return res;
  }
  ...
  _getGameDetailsObj(gameLists, gameDetails) {
    ...
    for (let i = 0; i < gameLists.length; i++) {
      if (_.isEmpty(gameDetails[0]) || gameLists[i].result.length !== gameDetails[i].items.item.length) {
  ...
  getBgInfoBasedOnPage(callback) {
    ...
    tasks.push({
      ...
      result: '',
      url: `${this.url}${_this.currentPage}?${this.filterParams}`
    });
    ...
    function iteratee(o, cb) {
      ...
      request(o.url, function (error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          let gameMeta = _this._getGameMeta($);
          ...
          if (gameNames.length === gameThumbnails.length && gameThumbnails.length === ratings.length) {
            for (let i = 0; i < gameNames.length; i++) {
              ...
              let {thumbnail} = gameThumbnails[i];
              let {bgglink, gameId} = gameMeta[i];
  ...
  getGameDetailsWraper(callback) {
    ...
    this.getBgInfoBasedOnPage((err, gameListsByPage) => {
      ...
      gameListsByPage.forEach((o, index) => {
        console.log(`Processing page ${_this.currentPage}`);
